...
if(cname != "None"){
	...
	draw_spaced(cname, name_box_text_x,name_box_text_y, c)
...
/*if(portrait[page] != -1){
	...
	draw_sprite_ext(portrait_frame, 1, pos_x-portraitWidth+1, pos_y+3, scale,scale, 0, $c6b3b8, 1);
	
	draw_sprite_ext(spr_player_portrait, portrait[page], pos_x-portraitWidth, pos_y+3, portraitscale,portraitscale, 0, c_white, 1);

	#region Idle Animated Portrait
if(type[page] == 1 or charCount >= str_len) {
	if(portrait_idle[page] != -1){
		var posx = pos_x-portraitWidth; var posy = pos_y+3;
		...
		draw_sprite_ext(portrait_idle[page], portrait_idle_c, posx, posy, portraitscale,portraitscale, 0, $c6b3b8, 1);	
	}
}
#endregion

	draw_sprite_ext(portrait_frame, 0, pos_x-portraitWidth+1, pos_y+3, scale,scale, 0, $c6b3b8, 1);
}*/
...
if(type[page] == 1){
	//Variables we need
	var col = default_col, tp = text[page], tpl = array_length(tp), txtwidth = 4*boxWidth-(x_buffer);
	...
	repeat(tpl){
		...
		var ctext = "* "+tp[ii];
		draw_set_color(col);
		draw_text_ext(xx, yy+((ii+iy)*stringHeight), ctext, -1, 200);
		//draw_spaced(ctext, xx, yy+((ii+iy)*stringHeight), col)
		
		if(string_width(ctext) > 200) { iy++; }
		ii++; 
	}
} 
#endregion
...
#region TYPE 0: NORMAL DIALOGUE
	...
	var bp_array = breakpoints, txtwidth = boxWidth-(x_buffer), char_max = txtwidth div charSize; 
	...
	repeat(charCount){
		...
		switch(effect){
			case 0:	//normal
				draw_text_color(xx + (cx), yy+(cy*stringHeight), letter, col, col, col, col, 1);
				break;
			
			case 1:	//shakey
				draw_text_color(xx + (cx)+random_range(-1,1), yy+(cy*stringHeight)+random_range(-1,1), letter, col, col, col, col, 1);
				break;
			...
			case 3: //colour shift
				...
				draw_text_color(xx + (cx), yy+(cy*stringHeight), letter, c1, c1, c2, c2, 1);
				break;
			...
			case 4: //wave AND colour shift
				...
				draw_text_color(xx + (cx), yy+(cy*stringHeight)+shift, letter, c1, c1, c2, c2, 1);
				break; 
			...
			case 5: //spin
				...
				draw_text_transformed_color(xx + (cx)+mv, yy+(cy*stringHeight)+(stringHeight/2), letter, 1, 1, shift*20, col, col, col, col, 1);
			...
			case 6: //pulse
				...
				var mv = charSize/2;
				draw_set_valign(fa_middle); draw_set_halign(fa_middle);
				draw_text_transformed_color(xx + (cx)+mv, yy+(cy*stringHeight)+(stringHeight/2), letter, shift, shift, 0, col, col, col, col, 1);
				draw_set_valign(fa_top); draw_set_halign(fa_left);
				break;
			...
			case 7:	//flicker
				var so = t + cc;
				var shift = sin(so*pi*freq/room_speed);
				draw_text_color(xx + (cx), yy+(cy*stringHeight), letter, col, col, col, col, shift+random_range(-1,1));
				break; 
		}
		...
		cc += 1;
		oldcx = cx;
		cx += string_width(letter);
		oldc = draw_get_color();
		if keyboard_check(ord("Q"))
		{
			draw_set_color(c_yellow)
			draw_rectangle(xx + (oldcx)+1, yy+(cy*stringHeight)+8, xx + (cx)-2, yy+(cy*stringHeight)+8, false)
			draw_set_color(oldc);
		}
	}
	#endregion
